    ...
    IOptions<RabbitMqOptions> options) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await channel.BasicQosAsync(
            prefetchSize: 0,
            prefetchCount: rabbitOptions.Prefetch == 0 ? (ushort)8 : rabbitOptions.Prefetch,
            global: false,
        ...
        logger.LogInformation(
            "Query worker consuming queue {QueueName} with prefetch {Prefetch} and concurrency {Concurrency}",
            queueName,
